...
if '--train' in sys.argv:
  train, test = pickle.load(open('train.pkl', 'rb')), pickle.load(open('test.pkl', 'rb'))  
  
  Xs, ys = map(np.array, list(train))
  Xst, yst = map(np.array, list(test))

  for i in range(100):
    model.fit( Xs, ys, validation_split=0.2, epochs=1, batch_size=1, callbacks=[batch_callback] )
